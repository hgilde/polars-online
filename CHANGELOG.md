...
- **`docs/REGIMES.md`: what the detectors actually find**, generated by
  ...
  one; `corrchange`'s size against Wied, Krämer & Dehling's Table 1 at 2000
  replications and its power against their Table 2 (higher than theirs, and
  recorded as unexplained); `corrchange(window)` against `bocpd` on the same
  break, which is 29 rows and 51 flags per 1000 quiet rows against 98 rows
  and 0.68; and the Epps curve, where refresh time recovers 0.54 of a true
  0.8 and the lag inversion 0.76.
