...
**Kept for later: 25 items** (V22 closed with task 81; batch 1 below
closed fourteen, batch 2 thirteen), each with its reason in the tables
below -- no independent library oracle (C4, C19, C24; S12), a decision
needed first -- taken by the user on 2026-09-15 and recorded in
`docs/PLAN.md` task 80 (S1, S2, S3, S23, S27, S29, S30, S31, and C24's
shape) -- performance (P1-P5), documentation (D9, D10), and the V list
(V5, V7, V11, V24, V25); and one new observation, N4, which task 81 did
half of. D1 is excluded by the user. The user's own
...
| V5, V7, V11, V24, V25 | later: to confirm; none is a fix yet |
