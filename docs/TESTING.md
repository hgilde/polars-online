...
| 6. Expression ≡ bank | Done for every model, incl. grids and `.over()`. Since 2026-09-03 every expression call also warns that it is the in-memory spelling (PLAN §6); `test_expr.py::TestTheExpressionWarnsThatItRunsInMemory` holds every method to that, and the suite silences the warning globally in `pyproject.toml`. |
