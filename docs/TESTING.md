...
| T-D4 | ~~P3~~ **mostly done** | Coverage is measured and reported by `scripts/coverage.sh` (96% Python, 75%/73% Rust with the caveat above) and CI reports the Python figure non-gating. Mutation testing is wired up in `scripts/mutants.sh`. A first scoped pass over `clock.rs` (17 mutants) found one **missed** mutant — nothing distinguished `raw < 0.0` from `raw <= 0.0` on the backwards-clock branch, i.e. no Rust test pinned that a *repeated* clock value is a zero delta rather than a reset (under the default `Max` policy those differ by a whole `max_dclock` of decay). A Rust unit test was added and the file is now 16/16 caught. Still open: the full `online-core` pass (1645 mutants, hours) and making it periodic. |
