...
| T-D3 thread determinism | **Done** — `tests/test_portability.py` runs the bank in subprocesses at `POLARS_ONLINE_MAX_THREADS=1` and `=8` and requires identical output. |
...
| T-D3 | ~~P2~~ **done** | Determinism across parallelism: the bank is run in subprocesses at `POLARS_ONLINE_MAX_THREADS=1` and `=8` over six groups, and the outputs must be identical. |
