...
`kalman`, `huber`, `quantile`, `sgd`, `pa`, `ftrl`, `holt`) plus `ew_cov`, two
entry points with identical numerics (the expression plugin, the original
third, is dormant since 2026-09-03 — PLAN §6 and E34 below), chunk invariance
and out-of-sample-ness enforced by tests, release CI defined, defaults
...
| E34 | ~~P1~~ **done** | **Park the expression plugin.** `pl.col("y").online.<model>(...)` is dormant: out of the wheel (off-by-default cargo feature `expr-plugin` on `online-py`), out of the README, `pl.Expr.online` not registered and `po.online` not exported in a build without it; a call raises a `RuntimeError` naming `lf.online.fit_predict`. The code, `_expr.py` and every test stay — the static checks run in every build, the runtime ones skip with the reason, CI compiles the Rust side with `--all-features` — and `maturin develop --features expr-plugin` brings it back for a later look. `po.has_expr_plugin()` reports the build. | E33 fixed the query-shaped trap by adding the streaming spelling, which left two spellings with one set of numbers and two memory profiles (7.3 GB against 1.35 GB at 12M rows), and users read the expression as the natural one. The cause is polars' contract for a stateful user expression (whole column, either engine), which no plugin can change, so the spelling goes rather than the warning. Nothing lost but the one polars-guaranteed interface (CLAUDE.md rule 13): `df.online.fit_predict` is the in-memory call, `group=` is the parallel path `.over(group)` was, the numbers are the same bits. PLAN §6 has the design and the un-parking condition: a polars node that runs a user expression per morsel, in order, with state. |
