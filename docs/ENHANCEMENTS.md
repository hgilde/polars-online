...
| E4 | P1 | **Validate weights: reject negative values.** A negative weight currently corrupts state: `EwCov::update` silently no-ops when `λW + w ≤ 0`, while the per-target `r_j`/`w_j` update runs anyway with a negative denominator. (Found while writing `docs/TESTING.md`; see T-E3 there.) Either reject `w < 0` at the null-policy layer or define and implement semantics. | PLAN §3 defines `weight` but never constrains it. |
| E5 | P2 | **Surface `solve_failures`** (and jitter usage) as an output field or a bank-level counter query. It is tracked in every solving model but invisible to users. | PLAN §7: "never NaN silently, record `solve_failures`" — recorded, not reportable. |
