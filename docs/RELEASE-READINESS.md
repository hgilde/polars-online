...
public, and what makes its API safe to promise. How a release is actually cut
today, and where the one human click sits, is "The release gate (2026-09-06)"
near the end.
...
**2. Twenty-one spec constructors, ~200 named keyword parameters** (29 of
them the `CommonKwargs` every spec shares), plus four helper
modules (`po.corr`, `po.sim`, `po.gram`, `po.prep`) whose functions are read
the same way. All keyword-only (good — positional order is not API), but
every *name* is. `tests/api_surface.txt` pins them all, helper modules
included.
...
`state_schema4.rs`, `state_schema5.rs`), and a documented rule (hard rule 5)
to keep a previous-version loader. This is the model the rest should follow. Each
...
**2026-09-03: the expression plugin warns on every use** (`docs/PLAN.md` §6,
...
Five models (`deco`, `rcov`, `hmm`, `corrchange`, `bocpd`), four helper
modules' worth of new functions (`po.corr` with twenty, `po.sim.regimes`,
`po.prep.refresh_time`, `po.gram.from_row`), `group_close` and the
closed-group queue on every spec, and lagged co-moments on `ew_cov`. In
API terms:

- **`SCHEMA_VERSION` is 5**, and `state_schema5.rs` freezes it — a bank with
  an undrained closed row, an `ew_cov` with a partly filled lag ring, and one
  of each new model mid-stream. `state_schema4.rs` still loads, continues to
  the bit and re-saves as 5, which is hard rule 5 discharged.
- **Every new output field name is pinned** by `tests/api_surface.txt` (the
  snapshot now carries a `[helper modules]` section) and by
  `tests/test_golden_pipeline.py`, which fixes the numbers of a
  twenty-five-spec bank at three rows.
- **Three of the new defaults are measured, not chosen**, and the measurement
  is in `docs/REGIMES.md`: `bocpd`'s `robust_beta = 0.1` (above ~0.2 nothing
  is ever detected), `bocpd`'s `emission = "diag"`, and `rcov`'s automatic
  bandwidth. Changing one of them is a breaking change by the rule below.
- **Two of the three streaming paths still carry no guarantee** (CLAUDE.md
  ...
  that, and the batch adds no new polars API dependency beyond
  `LazyFrame.collect_batches`, which is already the floor.
- **What is still unpinned**: the closed-group frame's *column* names
  (`spec`, `group`, `session`, `rcov`, `bandwidth_used`, …) are as much API
  as an output field is, and only `test_closed_groups.py` reads them by
  name. They belong in the snapshot the next time it is regenerated by
  hand.

## The release gate (2026-09-06)

**The one human step in a release is approving the PyPI upload, and it is the
only step that cannot fail.** The `Pypi` environment has the owner as a
required reviewer, with self-review allowed — with a single reviewer,
preventing it would deadlock every release. The `publish to PyPI` job
declares that environment and `needs: [sdist, build, read-state]`, so the run
asks for approval only after all six wheels, the sdist and the cross-OS state
hand-off are green. The click is a button on the run page; GitHub emails when
one is waiting.

Everything on either side of it is machine work, and that is the point of
putting the gate where it is:

- **Before**: dispatching a rehearsal, re-running a failed wheel, cancelling a
  run, pushing the tag. All retryable, none of it reaching a user.
- **After**: nothing. The upload is the last step and the only irreversible
  one — PyPI never allows a version number to be reused, even after a yank.

A tag that turns out to be wrong is therefore *not* fixed by moving it. The
`release tags are immutable` ruleset covers `refs/tags/v*` with `deletion`
and `update` rules and an empty bypass list, so the owner is bound too; a bad
tag is left unapproved and the fix ships as the next version. That agrees with
PyPI's rule rather than fighting it: version numbers are single-use on both
sides.

What is deliberately *not* gated: the GitHub release job, which runs before
the approval so the wheels and CLI binaries are attached to the release page
while the decision is being made, and can be deleted and recreated if wrong;
and the Pages deploy in `ci.yml`, which republishes the API reference on every
push to `main` and is as reversible as the next push. `github-pages` briefly
carried a required reviewer on 2026-09-06 and no longer does.
