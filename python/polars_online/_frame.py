...
expression plugin, in the same query, is O(data): polars calls a user
expression once with its whole column, and its streaming engine collects the
column to do so (docs/PERFORMANCE.md section 11). This is polars' IO-plugin
mechanism (``polars.io.plugins.register_io_source``): the bank is registered
as a *source*, the kind of node the engine pulls batches from, and what
comes after it -- filters, selects, joins, ``sink_parquet`` -- is polars' own.
