...
cycle_profile pattern, a volume clock — each do what they say.
"""
...
def returns(rows, m=4):
    x = rows.select([f"x_{i + 1}" for i in range(m)]).to_numpy()
    return np.diff(x, axis=0)


def test_the_frames_have_the_documented_schema_and_lengths():
    out = two_state(n_blocks=3, rows_per_block=50)
    assert set(out) == {"rows", "truth_rows", "truth_blocks"}
    rows, truth, blocks = out["rows"], out["truth_rows"], out["truth_blocks"]
    assert rows.height == truth.height == 150
    ...
    assert truth.columns == ["t", "block", "state", "scale_mult", "mix"]
    assert blocks.columns == ["block", "state", "n_rows", "corr"]
    assert rows["entity"].unique().to_list() == ["sim"]
    assert rows["activity"].null_count() == 150, "no activity unless asked for"
    assert blocks["corr"][0].len() == 10, "vech of a 4x4"


def test_a_seed_reproduces_every_frame_byte_for_byte():
    a, b = two_state(n_blocks=3, rows_per_block=40), two_state(n_blocks=3, rows_per_block=40)
    for key in ("rows", "truth_rows", "truth_blocks"):
        assert a[key].equals(b[key]), key
    other = two_state(n_blocks=3, rows_per_block=40, seed=1)
    assert not a["rows"].equals(other["rows"])
...
def test_the_truth_blocks_are_the_states_matrices():
    out = two_state(n_blocks=4, rows_per_block=100)
...
def test_smooth_interpolates_and_every_matrix_on_the_way_is_valid():
    out = two_state(
        n_blocks=4, rows_per_block=200, design="smooth", smooth_rows=40, durations=[1, 1]
    )
...
def test_phi_is_recovered_from_the_lag_one_autocorrelation():
    out = sim.regimes(
        ...
        transition=[[1.0]],
        n_blocks=1,
        rows_per_block=40_000,
        phi=[0.6, 0.0],
        seed=3,
    )
    ret = returns(out["rows"], m=2)
...
def test_noise_makes_the_observed_return_an_ma_one():
    ...
    common = dict(states=[0.3], transition=[[1.0]], n_blocks=1, rows_per_block=20_000, seed=4)
    clean = returns(sim.regimes(2, noise=0.0, **common)["rows"], m=2)
    noisy = returns(sim.regimes(2, noise=2.0, **common)["rows"], m=2)
...
def test_the_volatility_state_scales_the_returns():
    ...
    assert np.allclose(
        out["truth_rows"]["scale_mult"].to_numpy(),
...
def test_the_diurnal_pattern_scales_the_off_diagonal():
    ...
    out = sim.regimes(
        ...
        rows_per_block=40_000,
        cycle_rows=4,
        cycle_profile=pattern,
        seed=6,
    )
    ret = returns(out["rows"], m=3)
    # Bar `t` of the session produced return `t` (the diff at index t-1
    # spans rows t-1 and t); compare the quiet phase with a busy one.
...
def test_the_session_and_the_clock_follow_their_parameters():
    out = two_state(n_blocks=2, rows_per_block=60, cycle_rows=20, activity=(100.0, 2.0))
    rows = out["rows"]
    assert rows["session"].to_list() == [t // 20 for t in range(120)]
    assert rows["activity"].null_count() == 0
    assert rows["activity"].min() > 0
    assert np.allclose(rows["clock"].to_numpy(), np.cumsum(rows["activity"].to_numpy()))
    # Without a volume the clock is the bar index.
    plain = two_state(n_blocks=1, rows_per_block=10)["rows"]
...
def test_async_rates_drop_ticks_and_the_epps_curve_rises():
    ...
    out = sim.regimes(
        ...
        transition=[[1.0]],
        n_blocks=1,
        rows_per_block=60_000,
        async_rates=[0.25, 0.25],
        seed=7,
    )
    rows = out["rows"]
    assert rows["x_1"].null_count() > 0, "a bar with no tick carries null"
    filled = rows.select(pl.col("x_1", "x_2").forward_fill()).drop_nulls()
    x = filled.to_numpy()
    curve = []
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"cycle_profile": [0.5]}, "one multiplier per cycle row"),
        ({"cycle_profile": [0.0] * 500}, r"must be in \(0, 1\]"),
        ({"async_rates": [1.0]}, "one expected report rate per series"),
        ({"activity": (0.0, 1.0)}, r"activity is \(mean, shape\)"),
        ({"n_blocks": 0}, "must be >= 1"),
    ],
)
def test_a_bad_call_is_refused_by_name(kw, message):
    opts = {"n_blocks": 2, "rows_per_block": 500, **kw}
